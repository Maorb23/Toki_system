...
Office add-ins require HTTPS task pane URLs. Serve this folder from `https://127.0.0.1:3000` or update every `https://127.0.0.1:3000` URL in `manifest.xml` to your own HTTPS host.

This repo uses `127.0.0.1` instead of `localhost` because some local development environments bind another service on the `localhost`/IPv6 route for port 3000. The Office add-in dev certificate generated by `office-addin-dev-certs` includes both `127.0.0.1` and `localhost`.
