...
def _unique_demo_receiver_name(org: Organization, base_name: str) -> str:
    candidate = base_name.strip() or "Gmail Demo Receiver"
...
def get_gmail_demo_participants(payload: dict) -> tuple[Organization, Employee, Employee, bool]:
    ...
    if receiver is None:
        safe_name = _unique_demo_receiver_name(org, receiver_name or receiver_email.split("@")[0] or "Gmail Demo Receiver")
        receiver = Employee.objects.create(
            organization=org,
            name=safe_name,
            email=receiver_email,
            role="Gmail demo receiver",
            communication_preferences={},
            pain_points=[],
            receiver_prompt="Demo receiver created from Gmail integration. Use a clear, respectful, specific message.",
        )
...
def analyze_gmail_draft(payload: dict, *, request: HttpRequest | None = None) -> tuple[Message, dict]:
